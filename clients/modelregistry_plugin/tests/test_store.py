...
class TestModelRegistryStore:
    ...
    def store(self):
        ...
        return ModelRegistryStore(
            "modelregistry://localhost:8080", "s3://bucket/artifacts"
        )
    ...
    def test_request_with_custom_properties(self, mock_request, store):
        ...
        response_data = store._request(
            "POST", "/test", json={"customProperties": {"key1": "value1"}}
        )
    ...
    def test_create_experiment_with_tags(self, mock_request, store):
        ...
        # check that the experiment id is as expected
        assert experiment_id == "exp-123"
    ...
    def test_get_experiment(self, mock_request, store):
        ...
        assert (
            experiment.artifact_location == "s3://bucket/artifacts/experiments/exp-123"
        )
    ...
    def test_update_run_info(self, mock_request, store):
        ...
        run_info = store.update_run_info(
    ...
    def test_log_batch(self, mock_request, store):
        ...
        )  # GET run + PATCH + metric POST + param POST
    ...
    def test_search_logged_models(self, mock_request, store):
        ...
        mock_response_artifacts.json.return_value = {
            "items": [{"id": "model-123", "name": "test-model"}]
        }
    ...
    def test_get_artifact_location(self, store):
        ...
        assert (
            store._get_artifact_location(response_with_external_id)
            == "s3://bucket/artifacts"
        )

        # When no externalId is provided, the store uses its artifact_uri as fallback
        assert (
            store._get_artifact_location(response_without_external_id)
            == "s3://bucket/artifacts"
        )
