...
var _ = Describe("TestGetCatalogFilterListHandler", func() {
	Context("testing catalog filter list handler", Ordered, func() {
		...
		It("should retrive filter option list", func() {
			...
			actual, rs, err := setupApiTest[CatalogFilterOptionEnvelope](http.MethodGet, "/api/v1/model_catalog/filter_options", nil, kubernetesMockedStaticClientFactory, requestIdentify, "kubeflow")
