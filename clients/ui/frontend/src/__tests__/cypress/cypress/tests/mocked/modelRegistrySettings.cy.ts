import { modelRegistrySettings, FormFieldSelector } from '../../pages/modelRegistrySettings';
  ...
  data: {
    items: [
      ...
      },
      {
        ...
        subjects: [
          { kind: 'User', apiGroup: 'rbac.authorization.k8s.io', name: 'admin-user' }
        ],
        roleRef: {
      ...
      },
      {
        ...
        subjects: [
          { kind: 'User', apiGroup: 'rbac.authorization.k8s.io', name: 'dora-user' }
        ],
        roleRef: {
          apiGroup: 'rbac.authorization.k8s.io',
          kind: 'Role',
      ...
      },
      {
        ...
        subjects: [
          { kind: 'Group', apiGroup: 'rbac.authorization.k8s.io', name: 'bella-team' }
        ],
        roleRef: {
          apiGroup: 'rbac.authorization.k8s.io',
          kind: 'Role',
...
const modelRegistryMock = mockModelRegistryKind({ name: 'model-registry', displayName: 'Model Registry', description: 'Main registry' });
const modelRegistryMockDora = mockModelRegistryKind({ name: 'model-registry-dora', displayName: 'Dora Registry', description: 'Dora registry' });
const modelRegistryMockBella = mockModelRegistryKind({ name: 'model-registry-bella', displayName: 'Bella Registry', description: 'Bella registry' });

const setupModelRegistryMocks = (registries: any[] = [modelRegistryMock, modelRegistryMockDora, modelRegistryMockBella]) => {
...
describe('Model Registry Settings', () => {
  ...
  //     setupModelRegistryMocks([
  //       mockModelRegistryKind({ name: 'model-registry' }),
  //       mockModelRegistryKind({ name: 'test-registry-2' }),
  //     ]);
  //     // Mock role bindings for model-registry
  //     cy.intercept('GET', '/model-registry/api/v1/settings/role_bindings*', {
  //       data: {
  //         items: [
  //           { metadata: { name: 'rb-1' }, modelRegistryName: 'model-registry' },
  //         ],
  //       },
  //     });
  //     modelRegistrySettings.visit();
  ...
  //       .click({ force: true });
  //     cy.url().should('include', '/model-registry-settings/permissions/model-registry');
  //     cy.contains('Manage model-registry permissions').should('exist');
  //   });

  //   it('Manage permission is disabled, when there is no rolebinding', () => {
  //     setupModelRegistryMocks([
  //       mockModelRegistryKind({ name: 'test-registry-3' }),
  //     ]);
  //     // No role bindings for test-registry-3
  //     cy.intercept('GET', '/model-registry/api/v1/settings/role_bindings*', { data: { items: [] } });
  //     modelRegistrySettings.visit();
  //     selectNamespaceIfPresent();
  //     modelRegistrySettings
  ...
  //       .findByText('Manage permissions')
  //       .trigger('mouseover');
  //     modelRegistrySettings.findManagePermissionsTooltip().should('exist');
  //   });
  // });
