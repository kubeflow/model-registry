...
class ExperimentRunCreate(BaseModel):
    """
    Represents an ExperimentRun belonging to an Experiment.
    """ # noqa: E501
    ...
    name: Optional[StrictStr] = Field(default=None, description="The client provided name of the experiment run. It must be unique among all the ExperimentRuns of the same type within a Model Registry instance and cannot be changed once set.")
    end_time_since_epoch: Optional[StrictStr] = Field(default=None, description="End time of the actual experiment run in milliseconds since epoch. Different from lastUpdateTimeSinceEpoch, which is registry resource update time.", alias="endTimeSinceEpoch")
    status: Optional[ExperimentRunStatus] = ExperimentRunStatus.RUNNING
    state: Optional[ExperimentRunState] = ExperimentRunState.LIVE
    owner: Optional[StrictStr] = Field(default=None, description="Experiment run owner id or name.")
    experiment_id: Annotated[str, Field(min_length=1, strict=True)] = Field(description="ID of the `Experiment` to which this experiment run belongs.", alias="experimentId")
    start_time_since_epoch: Optional[StrictStr] = Field(default=None, description="Start time of the experiment run in milliseconds since epoch. Different from createTimeSinceEpoch, which is registry resource creation time.", alias="startTimeSinceEpoch")
    __properties: ClassVar[List[str]] = ["customProperties", "description", "externalId", "name", "endTimeSinceEpoch", "status", "state", "owner", "experimentId", "startTimeSinceEpoch"]
