...
class TrustManager:
    ...
    def _get_tuf_cache_dir(self, tuf_url: str) -> Path:
    ...
    def _get_root_json_path(self, tuf_url: str) -> Path:
        """Get cached root.json path for bootstrap."""
        return self._get_tuf_cache_dir(tuf_url) / "root.json"

    def _get_trust_root_path(self, tuf_url: str) -> Path:
        """Get trusted_root.json cache path."""
        return self._get_tuf_cache_dir(tuf_url) / "root" / "targets" / "trusted_root.json"

    def _get_trust_config_path(self, tuf_url: str) -> Path:
        """Get trust_config.json cache path."""
        return self._get_tuf_cache_dir(tuf_url) / "trust_config.json"
    ...
    def initialize(
        ...
        """
        config_path = self._get_trust_config_path(tuf_url)
        ...
        try:
            ...
            trusted_root = self._transform_and_fix_trust_root(trusted_root, fulcio_url, rekor_url, tsa_url)
    ...
    def _download_trusted_root(
        ...
        try:
            ...
            # Try cache first
            root_cache_path = self._get_root_json_path(tuf_url)
    ...
    def _transform_and_fix_trust_root(
        self,
        trusted_root: dict,
        fulcio_url: str | None = None,
        rekor_url: str | None = None,
        ...
        trusted_root = self._transform_checkpoint_to_logid(trusted_root)

        logger.info("Fixing internal URLs to external routes...")
        return self._fix_trusted_root_urls(trusted_root, fulcio_url, rekor_url, tsa_url)
    ...
    @staticmethod
    def _fix_trusted_root_urls(
        trusted_root: dict,
        fulcio_url: str | None = None,
        rekor_url: str | None = None,
        tsa_url: str | None = None,
    ) -> dict:
        """Override internal URLs with external routes if provided."""
        # Implementation would fix URLs in trusted_root
        return trusted_root
    ...
    def _cache_trust_config(self, tuf_url: str, trust_config: dict):
        ...
        trust_root_path = self._get_trust_root_path(tuf_url)
        trust_config_path = self._get_trust_config_path(tuf_url)
