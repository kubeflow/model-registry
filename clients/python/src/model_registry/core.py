...
class ModelRegistryAPIClient:
    ...
    async def get_model_version_by_params(
        ...
        """
        if not (external_id or (registered_model_id or name)):
            msg = "either external ID or registered model ID and name must be provided"
    ...
    async def get_model_artifact_by_params(
        ...
        Raises:
            StoreException: If either external ID or model version ID and model artitact name are not provided.
        """
        async with self.get_client() as client:
            try:
