...
def test_upsert_registered_model(
    ...
    rm_proto = mr_api._store.store.get_context_by_type_and_name(
...
def test_get_registered_model_by_id(
    ...
    rm_id = mr_api._store.store.put_contexts([registered_model.proto])[0]
...
def test_get_registered_models(
    ...
    rm1_id = mr_api._store.store.put_contexts([registered_model.proto])[0]
    registered_model.proto.name = "model2"
    rm2_id = mr_api._store.store.put_contexts([registered_model.proto])[0]
...
def test_upsert_model_version(
    ...
    mv_proto = mr_api._store.store.get_context_by_type_and_name(
...
def test_get_model_version_by_id(mr_api: ModelRegistryAPIClient, model_version: Mapped):
    ...
    ctx_id = mr_api._store.store.put_contexts([model_version.proto])[0]
...
def test_get_model_version_by_name(
    ...
    model_version.proto.name = f"1:{model_version.proto.name}"
    mv_id = mr_api._store.store.put_contexts([model_version.proto])[0]
...
def test_get_model_versions(
    ...
    mv1_id = mr_api._store.store.put_contexts([model_version.proto])[0]
    model_version.proto.name = f"{rm_id}:version2"
    mv2_id = mr_api._store.store.put_contexts([model_version.proto])[0]

    mr_api._store.store.put_parent_contexts(
        [
...
def test_upsert_model_artifact(
    ...
    ma_proto = mr_api._store.store.get_artifact_by_type_and_name(
...
def test_upsert_duplicate_model_artifact_with_different_version(
    ...
    ma_protos = mr_api._store.store.get_artifacts_by_id([int(ma1.id), int(ma2.id)])
...
def test_get_model_artifact_by_id(mr_api: ModelRegistryAPIClient, model: Mapped):
    ...
    id = mr_api._store.store.put_artifacts([model.proto])[0]
    id = str(id)
...
def test_get_model_artifact_by_model_version_id(
    ...
    model.proto.name = f"test_prefix:{model.proto.name}"
    ma_id = mr_api._store.store.put_artifacts([model.proto])[0]

    mr_api._store.store.put_attributions_and_associations(
...
def test_get_all_model_artifacts(mr_api: ModelRegistryAPIClient, model: Mapped):
    ...
    ma1_id = mr_api._store.store.put_artifacts([model.proto])[0]
    model.proto.name = "test_prefix:model2"
    ma2_id = mr_api._store.store.put_artifacts([model.proto])[0]
