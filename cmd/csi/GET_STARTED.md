...
## Multi-Profile / Multi-Tenant Usage

`ClusterStorageContainer` is a cluster-scoped KServe resource, so a multi-profile Kubeflow cluster cannot keep a different `MODEL_REGISTRY_BASE_URL` per namespace. The configured base URL is therefore only a shared fallback for short-form URIs such as `model-registry://iris/v1`.

For multi-profile deployments, deploy Model Registry into each profile namespace and use a fully qualified Model Registry service URL in every `InferenceService` `storageUri`.

### Deploy Model Registry + CSI per profile

The repository includes a Kustomize overlay that composes the database-backed Model Registry deployment with the CSI `ClusterStorageContainer`:

```bash
kubectl apply -k manifests/kustomize/overlays/per-namespace-csi
```

Before applying it, update the `namespace:` field in `manifests/kustomize/overlays/per-namespace-csi/kustomization.yaml` to the target profile namespace. The overlay uses the MySQL-backed `../db` manifests by default; switch the resource to `../postgres` if you want PostgreSQL instead.

> [!NOTE]
> The `ClusterStorageContainer` created by this overlay is still shared cluster-wide. If you apply the overlay for more than one profile, the most recently applied namespace becomes the fallback `MODEL_REGISTRY_BASE_URL`.

### Use embedded Model Registry URLs in `storageUri`

Use this URI format to force the CSI storage initializer to query the Model Registry service in the correct profile namespace:
...
The following `InferenceService` uses the embedded URL pattern so that a workload in one namespace can resolve models from the intended profile-specific Model Registry deployment:

```bash
kubectl apply -n profile-alpha -f - <<EOF
apiVersion: "serving.kserve.io/v1beta1"
kind: "InferenceService"
metadata:
  name: "iris-model"
spec:
  predictor:
    model:
      modelFormat:
        name: sklearn
      storageUri: "model-registry://model-registry-service.profile-alpha.svc.cluster.local:8080/iris/v1"
EOF
```

The example multi-profile `ClusterStorageContainer` manifest is available at `manifests/kustomize/options/csi/clusterstoragecontainer-multiprofile.yaml`. Use it as a reference when documenting or customizing a shared fallback `MODEL_REGISTRY_BASE_URL`.
