...
func TestArtifactFilterQuery(t *testing.T) {
	...
	t.Run("GetArtifacts with explicit experimentId.string_value filter", func(t *testing.T) {
		filterQuery := fmt.Sprintf(`experimentId.string_value = "%s"`, *createdExperiment2.Id)
		listOptions := api.ListOptions{
			PageSize:    apiutils.Of(int32(100)),
			FilterQuery: &filterQuery,
		}
