...
func TestQueryBuilderPropertyTypeSuffix(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedValueType: StringValueType,
			description:       "Should handle experimentId as string property",
		},
		{
			name:              "Artifact experimentRunId property",
			entityType:        EntityTypeArtifact,
			...
			expectedValueType: StringValueType,
			description:       "Should handle experimentRunId as string property",
		},
		{
			name:              "Artifact experimentId with explicit type",
			entityType:        EntityTypeArtifact,
			restEntityType:    RestEntityModelArtifact,
			query:             `experimentId.string_value = "exp-123"`,
			expectedSQL:       "experiment_id",
			expectedValueType: StringValueType,
			description:       "Should handle experimentId with explicit string_value type",
		},
	}
	...
	for _, artifactType := range artifactTypes {
		for _, prop := range properties {
			t.Run(fmt.Sprintf("%s_%s", artifactType.name, prop.name), func(t *testing.T) {
				...
				if propDef.ValueType != StringValueType {
					t.Errorf("Expected %s to be StringValueType, got %s", prop.propertyName, propDef.ValueType)
				}
				...
				if propRef.ValueType != StringValueType {
					t.Errorf("Expected property reference type %s, got %s", StringValueType, propRef.ValueType)
				}

				t.Logf("✅ %s.%s correctly mapped to %s (%s)", artifactType.name, prop.propertyName, prop.dbColumn, StringValueType)
			})
		}
	}
}
