...
func TestMapRegisteredModelPropertiesEmbedMD(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			source: &openapi.RegisteredModel{
				Owner:       &owner,
				Description: *openapi.NewNullableString(&description),
				State:       &state,
				Language:    language,
				Tasks:       tasks,
				Provider:    *openapi.NewNullableString(&provider),
				Logo:        *openapi.NewNullableString(&logo),
				License:     *openapi.NewNullableString(&license),
				LicenseLink: *openapi.NewNullableString(&licenseLink),
				LibraryName: *openapi.NewNullableString(&libraryName),
				Maturity:    *openapi.NewNullableString(&maturity),
				Readme:      *openapi.NewNullableString(&readme),
			},
			expected: &[]models.Properties{
				{
					Name:             "owner",
