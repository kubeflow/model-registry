...
func (c *ModelCatalogServiceAPIController) ListModelArtifacts(w http.ResponseWriter, r *http.Request) {
	sourceIdParam := chi.URLParam(r, "source_id")
	modelNameParam := chi.URLParam(r, "model_name")
	result, err := c.service.ListModelArtifacts(r.Context(), sourceIdParam, modelNameParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *ModelCatalogServiceAPIController) ListModels(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	...
	result, err := c.service.ListModels(r.Context(), sourceParam, qParam, pageSizeParam, model.OrderByField(orderByParam), model.SortOrder(sortOrderParam), nextPageTokenParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *ModelCatalogServiceAPIController) ListSources(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	...
	sortOrderParam := query.Get("sortOrder")
	nextPageTokenParam := query.Get("nextPageToken")
	result, err := c.service.ListSources(r.Context(), nameParam, pageSizeParam, model.OrderByField(orderByParam), model.SortOrder(sortOrderParam), nextPageTokenParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
